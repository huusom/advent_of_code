...
let testFirst(source, phases, expected) =
    ...
    let phs = phases |> Library.split ',' |> Seq.map (int)
...
let testSecond(source, phases, expected) =
    let prg = Program.load source
    let phs = phases |> Library.split ',' |> Seq.map (int) |> Seq.toList
    Assert.Equal(expected, second prg phs)


[<Fact>]
let evaluate() = 
    let prg = File.ReadAllText "input/d07.txt" |> Program.load
    let act = Library.perms [0;1;2;3;4] |> Seq.map (first prg) |> Seq.max
